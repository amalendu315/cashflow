...
public class PaymentRequestDetailsViewModel
{
    ...
    public RelatedPaymentRequestViewModel? ParentRequest { get; set; }

    public IReadOnlyList<RelatedPaymentRequestViewModel> ChildRequests { get; set; }
        = new List<RelatedPaymentRequestViewModel>();

    public PaymentRequestLedgerOutflowViewModel? LedgerOutflow { get; set; }

    public bool IsAdminView { get; set; }

    public bool HasParent => ParentRequest is not null;

    public bool HasChildren => ChildRequests.Any();

    public bool HasLedgerOutflow => LedgerOutflow is not null;
    ...
    public string ReviewedByDisplay =>
        string.IsNullOrWhiteSpace(ReviewedByName)
            ? "—"
            : $"{ReviewedByName} ({ReviewedByEmail})";
